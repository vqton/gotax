...
r.GET("/reset-password", func(c *gin.Context) {
	...
	zap.L().Info("GoTax GL server (PG) starting", zap.String("port", cfg.ServerPort))
		webSrv, err := web.NewServer([]string{"dashboard", "users", "journal-entries", "coa", "customers", "suppliers", "company", "exchange-rates", "periods", "cash-receipts", "cash-payments", "cash-transfers", "legacy"})
		if err != nil {
