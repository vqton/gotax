...
r.GET("/reset-password", func(c *gin.Context) {
	...
	handler.RegisterRoutesWithCompany(r, h, companyH, taxH, cashH, bankH, purchaseH, saleH, whH, faH, pwH, recH, budH, ccdcH, costH, keeperH, authMW, adminMW)
	...
	keeperSvcMem := service.NewWarehouseKeeperService(memKeeperRepo, memWhRepo, memItemRepo, memBalRepo)
	keeperHMem := handler.NewWarehouseKeeperHandler(keeperSvcMem)
	handler.RegisterRoutesWithCompany(r, h, companyH, taxH, cashH, bankH, purchaseH, saleH, whH, faHMem, pwHMem, recHMem, budHMem, ccdcHMem, costHMem, keeperHMem, authMW, adminMW)
