...
func NewACBParser() *ACBParser {
	return &ACBParser{}
}

func (p *ACBParser) GetBankCode() string {
	return "ACB"
}

func (p *ACBParser) Validate(data []byte) error {
	...
	header := records[0]
	if len(header) < 5 {
		return fmt.Errorf("ACB CSV must have at least 5 columns")
	}
	return nil
}

func (p *ACBParser) Parse(data []byte) ([]domain.BankTransaction, error) {
	...
	for i, record := range records {
		...
		debit, err := parseAmountACB(record[3])
		if err != nil {
			return nil, fmt.Errorf("invalid debit amount at row %d: %w", i+1, err)
		}
		txn.Debit = debit
		credit, err := parseAmountACB(record[4])
		if err != nil {
...
func parseAmountACB(s string) (float64, error) {
