...
func TestVTBParser_GetBankCode(t *testing.T) {
	parser := NewVTBParser()
	if parser.GetBankCode() != "VTB" {
		t.Errorf("expected VTB, got %s", parser.GetBankCode())
	}
}

func TestVTBParser_Validate(t *testing.T) {
	parser := NewVTBParser()
...
func TestVTBParser_Parse(t *testing.T) {
	parser := NewVTBParser()
