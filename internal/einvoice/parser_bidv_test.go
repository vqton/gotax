...
func TestBIDVParser_GetBankCode(t *testing.T) {
	parser := NewBIDVParser()
	if parser.GetBankCode() != "BIDV" {
		t.Errorf("expected BIDV, got %s", parser.GetBankCode())
	}
}

func TestBIDVParser_Validate(t *testing.T) {
	parser := NewBIDVParser()
	validCSV := []byte(`Date,TransactionID,Description,Debit,Credit,Balance,Reference
2026-08-10,TXN001,Payment,1000000,,5000000,REF001
2026-08-11,TXN002,Receipt,,500000,5500000,REF002`)
...
func TestBIDVParser_Parse(t *testing.T) {
	parser := NewBIDVParser()
	csvData := []byte(`Date,TransactionID,Description,Debit,Credit,Balance,Reference
2026-08-10,TXN001,Payment,1000000,,5000000,REF001
2026-08-11,TXN002,Receipt,,500000,5500000,REF002`)
