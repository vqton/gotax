...
func TestACBParser_GetBankCode(t *testing.T) {
	parser := NewACBParser()
	if parser.GetBankCode() != "ACB" {
		t.Errorf("expected ACB, got %s", parser.GetBankCode())
	}
}

func TestACBParser_Validate(t *testing.T) {
	parser := NewACBParser()
...
func TestACBParser_Parse(t *testing.T) {
	parser := NewACBParser()
