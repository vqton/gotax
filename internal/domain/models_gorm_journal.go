...
type JournalEntryGORM struct {
	...
	CompanyID       string    `gorm:"column:company_id;size:36;index" json:"companyId"`
	...
	PeriodID        string    `gorm:"column:period_id;size:36;index" json:"periodId"`
	...
	CreatedBy       string    `gorm:"column:created_by;size:36;index" json:"createdBy"`
...
type JournalLineGORM struct {
	ID            uint      `gorm:"primaryKey;autoIncrement" json:"id"`
	EntryID       uint      `gorm:"column:entry_id;not null;index:idx_entry_lines,unique;constraint:OnDelete:CASCADE" json:"entryId"`
